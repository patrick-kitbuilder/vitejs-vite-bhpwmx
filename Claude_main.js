  ...
  download.onclick = function () {
    ...
    for (var i = 0; i < bins.children.length; i++) {
      ...
      for (var j = 0; j < b.children.length; j++) {
        var clonedChild = b.children[j].cloneNode(true);
      ...
      if (reader.result) {
        try {
          var svg = window.SvgNest.parsesvg(reader.result);
          {
            ...
            var backgroundColor = svg.style ? svg.style.backgroundColor : null;
            if (!backgroundColor && svg.getAttribute('style')) {
              var styleMatch = svg.getAttribute('style').match(/background-color:\s*(#[0-9a-fA-F]{6}|#[0-9a-fA-F]{3}|rgba?\([^)]+\));?/);
              backgroundColor = styleMatch ? styleMatch[1] : null;
            }
            if (backgroundColor) {
              wholeSVG.style.backgroundColor = backgroundColor;
            }

            // Append all original attributes
            Array.from(svg.attributes).forEach(attr => wholeSVG.setAttribute(attr.name, attr.value));
